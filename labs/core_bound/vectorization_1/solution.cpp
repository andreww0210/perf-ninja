...
simd_sequence_t transpose(std::vector<sequence_t> const &sequences) {
  ...
  for (int i = 0; i < simd_sequences.size(); i++)
...
result_t compute_alignment(std::vector<sequence_t> const &sequences1,
  ...
  simd_sequence_t simd_sequences1 = transpose(sequences1);
  simd_sequence_t simd_sequences2 = transpose(sequences2);
  ...
  for (unsigned col = 1; col <= simd_sequences2.size(); ++col) {
    ...
    for (unsigned row = 1; row <= simd_sequences1.size(); ++row) {
      ...
      for (int k = 0; k < sequence_count_v; ++k)
          best_cell_score[k] += (simd_sequences1[row - 1][k] == simd_sequences2[col - 1][k] ? match[k] : mismatch[k]);
